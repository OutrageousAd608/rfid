    ...
    switch (currentState) {
        ...
        case PAGE_RX_SENSING:
            ...
            if (rf_rx.count == 0) {
                ...
                LCD_WriteString("RECORDING PULSES...  ", 20, 100, Font_7x10, COLOR_TERM_DIM, BLACK);
            }
...
void UI_Update_Dynamic_Elements(void) {
    ...
    if (currentState == PAGE_TRANSMITTING) {
        if ((HAL_GetTick() % 10) == 0) { 
            char hex[3];
    ...
    if (currentState == PAGE_RX_SENSING) {
        if (rf_rx.is_capturing && rf_rx.count > 0) {
             // Only force redraw when state changes from Waiting to Recording
             static uint16_t last_status_check = 0;
             if (rf_rx.count > 0 && last_status_check == 0) {
                 LCD_WriteString("RECORDING PULSES...  ", 20, 100, Font_7x10, COLOR_TERM_DIM, BLACK);
                 last_status_check = 1;
             }
        }

        if (RF_Process()) { // Returns 1 if valid signal found (Timeout)
            int new_slot = Find_Free_Slot();
            if (new_slot != -1) {
                selected_slot_idx = new_slot;
                signal_db[new_slot].is_active = 1; 
                
                // COPY FULL WAVEFORM DATA to permanent storage
                signal_db[new_slot].length = rf_rx.count;
                // Cast from volatile to normal pointer for memcpy
                memcpy(signal_db[new_slot].timings, (void*)rf_rx.timings, sizeof(uint32_t) * rf_rx.count);
    ...
    switch (currentState) {
        ...
        case PAGE_MAIN:
            ...
            if (Button_IsPressed(btn_Rx, x, y)) {
                ...
                RF_Start_Capture(); // START HARDWARE
                ui_needs_update = 1;
            }
            break;
        ...
        case PAGE_OPTIONS:
            if (Button_IsPressed(btn_Opt_Tx, x, y)) {
                ...
                UI_Refresh(); 
                // Replay the Saved Signal from DB
                // Cast to volatile for the driver function
                RF_Transmit((volatile uint32_t*)signal_db[selected_slot_idx].timings, signal_db[selected_slot_idx].length);
            }
